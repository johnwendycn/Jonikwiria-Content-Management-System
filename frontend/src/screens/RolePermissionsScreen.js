...
// API Configuration
const API_BASE_URL = 'http://localhost:5000/api/role-permissions';
...
export default function RolePermissionsScreen() {
  ...
  const loadRelationData = async () => {
    try {
      ...
      console.error('Failed to load role-permission dependencies:', err);
    }
  };
  ...
  const fetchRolePermissions = useCallback(async () => {
    ...
    try {
      ...
      if (json.success) {
        ...
        setError(json.message || 'Failed to fetch role permissions.');
      }
    } catch (err) {
  ...
  const toggleSelectAll = () => {
    const allOnPage = rolePermissions.map(item => item.id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (json.success) {
        ...
        fetchRolePermissions();
        showAlert('Success', isEditMode ? 'Role permission mapping updated successfully.' : 'Permission assigned to role successfully.');
      } else {
  ...
  const handleDelete = (item) => {
    confirmAction(
      'Confirm Delete',
      `Are you sure you want to remove permission "${item.permission ? item.permission.name : 'Permission'}" from role "${item.role ? item.role.name : 'Role'}"?`,
      async () => {
        setLoading(true);
        try {
          ...
          if (json.success) {
            fetchRolePermissions();
            showAlert('Success', 'Role permission assignment removed successfully.');
          } else {
  ...
  const handleBulkDelete = () => {
    confirmAction(
      'Confirm Bulk Delete',
      `Are you sure you want to delete the ${selectedIds.size} selected role permission assignments?`,
      async () => {
        setLoading(true);
        try {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      ...
      <View style={styles.header}>
        <View>
          <Text style={styles.headerTitle}>Role Permissions Configurator</Text>
          <Text style={styles.headerSubtitle}>
            Configure authorizations mapping roles to permissions, assign allow/deny policies, specify ABAC conditions, and control validity dates.
          </Text>
        </View>
      ...
      {isGuideVisible && (
        <View style={styles.guideCard}>
          <Text style={styles.guideCardTitle}>📚 Guide to Role Permissions & ABAC</Text>
          <Text style={styles.guideCardText}>
            Ensure you understand the evaluation policies:
          </Text>
          ...
          <View style={styles.bulletList}>
            <View style={styles.bulletRow}>
              ...
              <Text style={styles.bulletText}>
                <Text style={{ fontWeight: '600' }}>🚫 Explicit Deny Overrides Allow:</Text> If a role assignment is marked as Deny (is_allowed = false), this overrides any Allowed assignments of that permission from other roles or parent roles.
              </Text>
            </View>
            <View style={styles.bulletRow}>
              <View style={[styles.bulletDot, { backgroundColor: '#065F46' }]} />
              <Text style={styles.bulletText}>
                <Text style={{ fontWeight: '600' }}>⚡ Attribute-Based Access Control (ABAC):</Text> Define custom JSON conditions in the conditions editor (e.g. {"{\"allowed_departments\": [\"IT\"]}"}) to dynamically restrict access based on user or resource attributes.
              </Text>
            </View>
            <View style={styles.bulletRow}>
              <View style={[styles.bulletDot, { backgroundColor: '#D97706' }]} />
              <Text style={styles.bulletText}>
                <Text style={{ fontWeight: '600' }}>⏱️ Temporal Lifecycles:</Text> Restrict permissions to certain date ranges. Permissions automatically become inactive before valid_from and after valid_until.
              </Text>
            </View>
            <View style={styles.bulletRow}>
              <View style={[styles.bulletDot, { backgroundColor: '#4F46E5' }]} />
              <Text style={styles.bulletText}>
                <Text style={{ fontWeight: '600' }}>🔒 Active Allowed Uniqueness:</Text> A role cannot have the same permission mapped to it in an active allowed state (allowed = true, valid_until is null) twice.
              </Text>
            </View>
          </View>
        </View>
      ...
      <View style={styles.controlBar}>
        <View style={styles.searchContainer}>
          <TextInput
            ...
            placeholder="Search mappings by role, permission, or reason..."
            value={search}
      ...
      {loading && rolePermissions.length === 0 && (
        <View style={styles.loaderContainer}>
          ...
          <Text style={styles.loaderText}>Loading role permission mappings...</Text>
        </View>
      )}
      ...
      {error && (
        <View style={styles.errorContainer}>
          ...
          <TouchableOpacity style={styles.retryBtn} onPress={fetchRolePermissions}>
      ...
      {!loading && !error && rolePermissions.length === 0 ? (
        <View style={styles.emptyContainer}>
          <Text style={styles.emptyText}>No role permission mappings configured in the database.</Text>
        </View>
      ) : (
        <>
          {isLargeScreen ? (
                ...
                <View style={styles.tableWrapper}>
                  ...
                  <View style={styles.tableRowHeader}>
                    <TouchableOpacity onPress={toggleSelectAll} style={styles.checkboxCol}>
                      <View style={[styles.checkbox, rolePermissions.length > 0 && rolePermissions.every(item => selectedIds.has(item.id)) && styles.checkboxChecked]}>
                        {rolePermissions.length > 0 && rolePermissions.every(item => selectedIds.has(item.id)) && <CheckedIcon />}
                      </View>
                    </TouchableOpacity>
                    ...
                    <View style={styles.cardHeader}>
                      <TouchableOpacity onPress={() => toggleSelectRow(item.id)} style={styles.mobileCheckboxContainer}>
                        ...
                        <Text style={[styles.statusNameText, { marginLeft: 8 }]} numberOfLines={1}>
                          Role: {item.role ? item.role.name : 'Missing Role'}
                        </Text>
                      </TouchableOpacity>
          ...
          <View style={styles.paginationContainer}>
            <Text style={styles.paginationText}>
              Showing <Text style={{ fontWeight: '600' }}>{rolePermissions.length}</Text> of{' '}
          ...
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>
                {isEditMode ? 'Edit Role Permission Assignment' : 'Grant Permission to Role'}
              </Text>
            ...
            <ScrollView style={styles.modalFormScroll}>
              ...
              <DropdownSelector
                label="Target Role Configuration *"
                options={roleOptions}
              ...
              <View style={styles.formGroup}>
                ...
                <TextInput
                  ...
                  multiline={true}
                  numberOfLines={3}
                  placeholder="Explain why this permission level is assigned to the role..."
